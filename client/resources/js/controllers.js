...
app.controller('esCtrl', function($scope,$http,InitialiseSSM,RunGoals,CreateForce) {
	aa=$scope;
	...
	function setpreviousNodePositions(){
		var i = $scope.ssm.nodes.length;
		$scope.previousNodePositions={};
		for (;i--;){			
			$scope.previousNodePositions[$scope.ssm.nodes[i].id]=[$scope.ssm.nodes[i].x,$scope.ssm.nodes[i].y]
		}		
	}
		...
		InitialiseSSM.getSSM()
			.then(function(result){ 
				...
				.then(function(){  						
						...
						$scope.force.start();
						$scope.force.on("tick",function(){$scope.force.doTick($scope);});
...
/* 		RunGoals.RunGoals()
			.then(function(result){
				...
				function(error){console.log("Error in getGoals",error}); */
		return;
